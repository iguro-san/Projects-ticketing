    ...
    <div class="bg-white rounded-lg shadow p-6 mb-6">
        ...
        <div class="bg-gradient-to-r from-yellow-50 to-orange-50 border border-yellow-300 rounded-lg p-4 mb-6" 
            ...
            <div class="mt-3 bg-yellow-200 rounded-full h-2">
                <div class="bg-yellow-500 rounded-full h-2 transition-all duration-1000" :style="'width: ' + progressPercent + '%'"></div>
            </div>
        </div>
        ...
        <div class="bg-gray-50 rounded-lg p-4 mb-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div><p class="text-sm text-gray-500">No. Registrasi</p><p class="font-mono font-bold">{{ $registration->registration_number }}</p></div>
                <div><p class="text-sm text-gray-500">Event</p><p class="font-semibold">{{ $registration->event->title }}</p></div>
                <div><p class="text-sm text-gray-500">Jenis Tiket</p><span class="px-2 py-1 bg-indigo-100 text-indigo-700 rounded-full text-xs font-medium">{{ $registration->ticketType->name }}</span></div>
                <div><p class="text-sm text-gray-500">Total</p><p class="text-2xl font-bold text-green-600">Rp {{ number_format($registration->ticketType->price, 0, ',', '.') }}</p></div>
            </div>
        </div>
            ...
            <form action="{{ route('payment.upload', $registration) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-4">
                    ...
                    <select name="payment_method" class="w-full border rounded-lg px-3 py-2 focus:outline-none focus:border-purple-600" required>
                        ...
                        <option value="BCA">BCA</option><option value="Mandiri">Mandiri</option><option value="BRI">BRI</option><option value="BNI">BNI</option><option value="Other">Lainnya</option>
                    </select>
                </div>
                <div class="mb-4">
                    <label class="block text-gray-700 mb-2 font-semibold">Bukti Transfer</label>
                    <input type="file" name="payment_proof" accept="image/*" class="w-full border rounded-lg px-3 py-2" required>
...
<script>
    document.addEventListener('alpine:init', () => {
        Alpine.data('countdownTimer', (s) => ({
            remaining: s, total: s,
            init() { if (this.remaining > 0) this.start(); },
            get timeDisplay() {
                if (this.remaining <= 0) return 'Kadaluarsa';
                const h = Math.floor(this.remaining/3600), m = Math.floor((this.remaining%3600)/60), sec = this.remaining%60;
                return `${String(h).padStart(2,'0')}:${String(m).padStart(2,'0')}:${String(sec).padStart(2,'0')}`;
            },
            get progressPercent() { return this.total ? Math.max(0,Math.min(100,(this.remaining/this.total)*100)) : 0; },
            start() { const i = setInterval(() => { if (this.remaining>0) this.remaining--; else { clearInterval(i); setTimeout(()=>location.reload(),3000); } }, 1000); }
        }));
    });
</script>
@endpush
